...
class MultiModelServer:
    ...
    async def infer(self, model_id: str, request: Any) -> AsyncGenerator[Any, None]:
        ...
        logger.info(f"➡️ Dispatching inference to engine: {engine.__class__.__name__} for model_id: {model_id}")
        try:
    ...
    async def chat_completions(self, model_id: str, request: Any) -> AsyncGenerator[Any, None]:
        ...
        if model_id not in self.models:
            ...
            raise ValueError(f"Model '{model_id}' not found")
        engine = self.models[model_id]
        logger.info(f"➡️ Dispatching chat completions to engine: {engine.__class__.__name__} for model_id: {model_id}")
        try:
    ...
    async def completions(self, model_id: str, request: Any) -> AsyncGenerator[Any, None]:
        ...
        if model_id not in self.models:
            ...
            raise ValueError(f"Model '{model_id}' not found")
        engine = self.models[model_id]
        logger.info(f"➡️ Dispatching completions to engine: {engine.__class__.__name__} for model_id: {model_id}")
        try:
