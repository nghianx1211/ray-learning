...
class MultiModelRouter:
    ...
    async def _setup_handle_map(self, model_deployments: List[DeploymentHandle]):
        for handle in model_deployments:
            try:
                # only call get_config()
                model_config = await handle.get_config.remote()

                # if return multiple models (MultiModelDeployment)
    ...
    def _get_handle(self, model_id: str) -> DeploymentHandle:
        if model_id not in self._handles:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, f"Model '{model_id}' not found."
            )
    ...
    async def infer(self, body: Dict[str, Any]) -> Response:
        ...
        if not model_id:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Missing model_id in request"
            )
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR, f"Inference failed: {str(e)}"
            )
        ...
        try:
            ...
            async with async_timeout.timeout(DEFAULT_LLM_ROUTER_HTTP_TIMEOUT):
                if request.stream:
                    # Streaming response
                    async def generate():
                        generator = handle.options(stream=True).chat_completions.remote(
                            request_dict
                        )
            ...
            raise HTTPException(
                ...
                f"Chat completions failed: {str(e)}",
            )
        ...
        try:
            ...
            async with async_timeout.timeout(DEFAULT_LLM_ROUTER_HTTP_TIMEOUT):
                if request.stream:
                    # Streaming response
                    async def generate():
                        generator = handle.options(stream=True).completions.remote(
                            request_dict
                        )
                        async for chunk in generator:
                            if isinstance(chunk, dict) and "error" in chunk:
            ...
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR, f"Completions failed: {str(e)}"
            )
    ...
    async def list_models(self):
        ...
        await self._init_completed.wait()
        return JSONResponse(content={"models": list(self._configs.values())})
    ...
    @classmethod
    def as_deployment(
        cls, model_configs: Optional[List[Dict[str, Any]]] = None
    ) -> serve.Deployment:
        ...
        if model_configs:
            min_replicas = max(
                1, int(len(model_configs) * DEFAULT_ROUTER_TO_MODEL_REPLICA_RATIO)
            )
