...
class MultiModelServer:
    ...
    async def _start_model(self, cfg: MultiModelConfig):
        ...
        engine = engine_cls(
            model_id=model_id, model_source=model_source, **cfg.engine_kwargs
        )
        await engine.start()
    ...
    async def infer(self, model_id: str, request: Any) -> AsyncGenerator[Any, None]:
        ...
        logger.info(
            f"Dispatching inference to engine: {engine.__class__.__name__} for model_id: {model_id}"
        )
        try:
    ...
    async def chat_completions(
        ...
        engine = self.models[model_id]
        logger.info(
            f"Dispatching chat completions to engine: {engine.__class__.__name__} for model_id: {model_id}"
        )
        try:
    ...
    async def completions(
        ...
        engine = self.models[model_id]
        logger.info(
            f"Dispatching completions to engine: {engine.__class__.__name__} for model_id: {model_id}"
        )
        try:
