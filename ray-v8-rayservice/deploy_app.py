...
def main():
    ...
    try:
        deploy_health_check()
        console.print(
            "[bold green]✓ Health check endpoint ready at /-/healthz[/bold green]"
        )
    except Exception as e:
        console.print(
            f"[bold red]Warning: Health check deployment failed: {e}[/bold red]"
        )
    ...
    if len(configs) == 1:
        ...
        console.print(
            f"[bold green]Deploying single model application: {model_id}[/bold green]"
        )
        ...
        console.print(
            f"[bold green]Application '{model_id}' deployed successfully![/bold green]"
        )
        console.print(
            f"[bold cyan] Endpoint: http://127.0.0.1:8000/{model_id}[/bold cyan]"
        )
    else:
        # Multiple models - deploy each as separate application
        console.print(
            f"[bold green]Deploying {len(configs)} separate applications in parallel[/bold green]"
        )
        ...
        with ThreadPoolExecutor(max_workers=len(apps_dict)) as executor:
            futures = [
                executor.submit(deploy_app, model_id, app)
