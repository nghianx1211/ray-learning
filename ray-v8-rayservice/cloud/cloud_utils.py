...
def is_local_path(path: str) -> bool:
    ...
    return path.startswith("/") or (
        not "://" in path and ("/" in path or path.startswith("."))
    )
...
class CloudFileSystem:
    ...
    def download_files(
        ...
        try:
            ...
            raise ImportError(
                "You must `pip install pyarrow` to download from cloud storage."
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully downloaded {downloaded_count} files from {bucket_uri}"
            )
...
def resolve_model_source(model_source: str, cache_dir: Optional[str] = None) -> str:
    ...
    elif is_huggingface_id(model_source) and not os.path.exists(
        os.path.expanduser(model_source)
    ):
    ...
    # Handle local filesystem paths (including ones that look like HF IDs but exist locally)
    elif is_local_path(model_source) or os.path.exists(
        os.path.expanduser(model_source)
    ):
        ...
        if os.path.exists(expanded_path):
            ...
            if os.path.isdir(expanded_path):
                ...
                logger.info(
                    f"Local model directory contains {len(files)} files/folders"
                )
